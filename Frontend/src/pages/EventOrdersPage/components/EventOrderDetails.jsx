    ...
    <Card title="Order Items" size="small" style={{ marginBottom: 16 }}>
      <Table
        ...
        columns={[
          ...
          { title: "Price/Packet", dataIndex: "price", key: "price", render: (p) => `₹${p}` },
          { title: "Qty/Packet", dataIndex: "quantity", key: "quantity" },
          { title: "Total/Packet", key: "total", render: (_, r) => `₹${(r.price * r.quantity).toFixed(2)}` },
        ]}
      />
    </Card>
  );
  ...
  return (
    <div style={{ padding: "10px" }}>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={8}><Text strong>Purpose:</Text> {record.purpose}</Col>
        <Col span={8}><Text strong>Packets:</Text> {record.packets || 1}</Col>
        <Col span={8}><Text strong>Discount/Packet:</Text> ₹{record.discount || 0}</Col>
      </Row>
      <Row gutter={16} style={{ marginBottom: 16 }}>
