...
const HomeExpensesPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (src, record) => {
        ...
        const subText =
          src === "cc_loan" && record.ccLoanId?.accountName
            ? ` (${record.ccLoanId.accountName})`
            : src === "credit_card" && record.creditCardId?.cardName
            ? ` (${record.creditCardId.cardName})`
            : "";
        return (
          <Tag
            icon={cfg.icon}
  ...
  return (
    <div style={{ padding: "8px 4px 40px", background: "#f8fafc", minHeight: "100vh" }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Tag color="orange" bordered={false} style={{ borderRadius: 8, fontWeight: 700, margin: 0, fontSize: 11 }}>{supplierPct || 4}%</Tag>
            </div>
            <div style={{ marginTop: 8 }}>
      ...
      {/* ── 4. CATEGORY FILTER PILL TABS ROW B ── */}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          {/* CC Loan Account Selector — shown only when paymentSource is cc_loan */}
          ...
          {/* Credit Card Selector — shown only when paymentSource is credit_card */}
