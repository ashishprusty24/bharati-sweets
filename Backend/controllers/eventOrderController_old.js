  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        ...
        console.log("Ô£à WhatsApp message sent successfully:", data);
      } catch (whatsappError) {
        console.error("ÔØî Failed to send WhatsApp message:", whatsappError);
      }
    ...
    try {
      ...
      if (updatedOrder.paidAmount >= updatedOrder.totalAmount) {
        ...
        try {
          ...
          const data = await response.json();
          console.log("Ô£à Final Invoice WhatsApp message sent:", data);
        } catch (whatsappError) {
          console.error("ÔØî Failed to send WhatsApp message:", whatsappError);
        }
      } else {
        ...
        try {
          ...
          const data = await response.json();
          console.log("Ô£à Partial Payment WhatsApp message sent:", data);
        } catch (whatsappError) {
          console.error(
            "ÔØî Failed to send WhatsApp partial payment:",
            whatsappError
          );
        }
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (order.paidAmount > 0) {
        message += ` Ôé╣${order.paidAmount} will be refunded.`;
      }
