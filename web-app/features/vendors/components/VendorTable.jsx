...
const VendorTable = ({ data, loading, vendorTypes = [], onEdit, onDelete, onPay, expandedRowRender }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "due",
      render: (due) => <Text strong style={{ color: "#ef4444" }}>₹{due || 0}</Text>,
    },
    {
      title: "Actions",
      key: "actions",
      render: (_, record) => (
        <Space>
          <Tooltip title="Make Payment">
            <Button type="primary" icon={<DollarOutlined />} onClick={() => onPay && onPay(record)}>Pay</Button>
          </Tooltip>
          <Tooltip title="Edit">
            <Button type="default" icon={<EditOutlined />} onClick={() => onEdit && onEdit(record)} />
          </Tooltip>
          <Popconfirm title="Delete this vendor?" onConfirm={() => onDelete && onDelete(record._id)}>
