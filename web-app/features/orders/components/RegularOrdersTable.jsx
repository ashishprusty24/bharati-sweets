  ...
  const columns = [
    {
      ...
      dataIndex: "_id",
      key: "_id",
      render: (id) => <Text strong style={{ color: "#10b981" }}>#{id ? id.substring(id.length - 6).toUpperCase() : ""}</Text>,
    },
    {
      title: "Customer",
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space direction="vertical" size={4}>
          <Text strong style={{ color: "#10b981", fontSize: 16 }}>₹{record.payment?.amount || 0}</Text>
          {getPaymentMethodTag ? getPaymentMethodTag(record.payment?.method) : <Tag>{record.payment?.method}</Tag>}
        </Space>
      ),
    },
    {
