...
const EventPaymentModal = memo(({ visible, order, paymentMethods = [], onCancel, onOk, loading }) => {
  ...
  useEffect(() => {
    if (visible && order) {
      form.resetFields();
      form.setFieldsValue({
        amount: (order.totalAmount || 0) - (order.advancePaid || order.paidAmount || 0),
        method: "cash",
      });
    }
  }, [visible, order, form]);
            ...
            if (method === "phonepay" || method === "gpay") {
              return (
                <div style={{ textAlign: "center", margin: "20px 0" }}>
                  <Text strong>Scan to Pay ₹{form.getFieldValue("amount") || (order?.totalAmount || 0) - (order?.advancePaid || order?.paidAmount || 0)}</Text>
