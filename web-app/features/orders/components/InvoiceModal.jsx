...
const InvoiceModal = ({ visible, order, onCancel }) => {
  ...
  const subtotalPerPacket = (order.items || []).reduce((sum, item) => sum + ((item.price || item.unitPrice || 0) * (item.quantity || 1)), 0);
      ...
      <div ref={invoiceRef} style={{ padding: isMobile ? "16px" : "40px", background: "#fff", color: "#333", fontSize: isMobile ? "12px" : "14px" }}>
        ...
        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", borderBottom: "2px solid #333", paddingBottom: isMobile ? 12 : 20, marginBottom: isMobile ? 16 : 30, gap: isMobile ? 8 : 0 }}>
          ...
          <div style={{ textAlign: isMobile ? "left" : "right" }}>
            ...
            <Text style={{ fontSize: isMobile ? 11 : 14 }}>No: INV-{order._id ? order._id.slice(-6).toUpperCase() : ""}</Text><br />
        ...
        <Row gutter={isMobile ? 16 : 40} style={{ marginBottom: isMobile ? 16 : 30 }}>
          <Col xs={24} sm={12} style={{ marginBottom: isMobile ? 12 : 0 }}>
            ...
            <Text>{order.customerPhone || order.phone}</Text><br />
            <Text>{order.address}</Text>
          </Col>
          <Col xs={24} sm={12} style={{ textAlign: isMobile ? "left" : "right" }}>
            <Text strong>EVENT DETAILS:</Text><br />
            <Text>{order.purpose}</Text><br />
            <Text>Date: {dayjs(order.eventDate || order.deliveryDate).format("MMM D, YYYY")}</Text><br />
        ...
        <Table
          ...
          columns={[
            ...
            { title: "Price", dataIndex: "price", key: "price", align: "right", render: (p, item) => `₹${((p || item.unitPrice || 0) * item.quantity).toFixed(2)}`, width: isMobile ? 70 : undefined },
          ]}
        />
        ...
        <div style={{ marginTop: isMobile ? 16 : 30, paddingLeft: isMobile ? 0 : "50%" }}>
          ...
          <Row style={{ marginBottom: 8 }}>
            ...
            <Col span={10} style={{ textAlign: "right" }}><Text strong style={{ fontSize: isMobile ? 14 : 16 }}>₹{(order.totalAmount || totalAmount).toFixed(2)}</Text></Col>
          </Row>
          <Row style={{ marginBottom: 8 }}>
            <Col span={14}><Text>Advance Paid:</Text></Col>
            <Col span={10} style={{ textAlign: "right", color: "#10b981" }}><Text style={{ color: "inherit" }}>₹{(order.advancePaid || order.paidAmount || 0).toFixed(2)}</Text></Col>
          </Row>
          ...
          <Row>
            ...
            <Col span={10} style={{ textAlign: "right" }}><Text strong style={{ fontSize: isMobile ? 15 : 18, color: "#d4380d" }}>₹{((order.totalAmount || totalAmount) - (order.advancePaid || order.paidAmount || 0)).toFixed(2)}</Text></Col>
          </Row>
        </div>
